...
export class CuponController {
    ...
    async createCupon(id_establecimiento: number, cupon: any) {
        return await api.post(`/api/establecimiento/v1/cupones/${id_establecimiento}`, {
            body: createFormData(cupon),
        }, {}, this.token)
    }

    async getCuponesEstablecimiento(id_establecimiento: number) {
        return await api.get<CuponesEstablecimientoResponse>(`/api/establecimiento/v1/cupones/${id_establecimiento}`, {}, this.token)
    }
