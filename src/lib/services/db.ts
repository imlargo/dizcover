...
export class DatabaseController {
	async getEstablecimiento(id: string | number): Promise<Establecimiento> {
		const response = await $api.get(`/api/establecimiento/${id}`);
		return response;
	}
	...
	async getEstablecimientos(): Promise<Establecimiento[]> {
		const response = await $api.get(`/api/establecimiento/`);
		return response;
	}
	...
	async getImagenesEstablecimiento(id: string | number): Promise<string[]> {
		const response = await $api.get(`/api/establecimiento/${id}/imagenes`);
		return response;
	}

	async getImagenesEvento(id: string | number): Promise<string[]> {
		const response = await $api.get(`/api/evento/${id}/imagenes`);
		return response;
	}
	...
	async getCordenadasEstablecimiento(
		...
		const response = await $api.get(`/api/establecimiento/${idEstablecimiento}/coordenadas/`);
		return response;
	}

	async getPricingEvento(idEvento: string | number): Promise<PricingEvento[]> {
		const response = await $api.get(`/api/evento/asientos/${idEvento}/`);
		return response;
	}

	async getEventosEstablecimiento(idEstablecimiento: string | number): Promise<Evento[]> {
		const response = await $api.get(`/api/evento/establecimiento/${idEstablecimiento}`);
		return response;
	}

	async getHorariosEstablecimiento(idEstablecimiento: string | number): Promise<Horario[]> {
		const response = await $api.get(`/api/establecimiento/${idEstablecimiento}/horarios/`);
		return response;
	}
}
